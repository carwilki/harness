...
report = hjm.getReport("WM_LABOR_MSG_DTL_CRIT")
all_records_not_apearing_in_either_v1_pre_v2 = """with rf_only as ((
  --create a set of locid and msdid
  select rf.location_id,rf.WM_LABOR_MSG_DTL_CRIT_ID from qa_refine.WM_LABOR_MSG_DTL_CRIT rf
  where rf.LOCATION_ID =1288 or rf.LOCATION_id=1186)
except
( --remove all the ids that are present in the pre_table
  select sp.location_id,r.LABOR_MSG_DTL_CRIT_ID from qa_legacy.SITE_PROFILE sp
inner join qa_raw.WM_LABOR_MSG_DTL_crit_PRE r on sp.store_nbr = r.DC_NBR
...
final as ((
  ...
  inner join rf_only o on rf.location_id = o.location_id and rf.WM_LABOR_MSG_DTL_CRIT_ID = o.WM_LABOR_MSG_DTL_CRIT_ID)
except(
