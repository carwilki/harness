...
from pyspark.sql.functions import col, greatest, max, min
...
snapshot_name = "WM_TRAILER_VISIT"
...
records_not_inserted = pre_keys.exceptAll(refine_keys).exceptAll(v1_keys).cache()
...
records_in_v1_not_in_pre = v1_keys.exceptAll(pre_keys).cache()
records_in_v2_not_in_pre = v2_keys.exceptAll(pre_keys).cache()
# min_test_timstamp = v1.select(min('WM_CREATE_TSTMP')).collect()[0][0]
# max_test_timstamp = v2.select(max('WM_CREATE_TSTMP')).collect()[0][0]
print(
    "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n"
)
...
print(f"pre-table post join site profile                    {delta_pre.count()}")
# print(f"min and max snapshot timestamps:                    {min_test_timstamp}, {max_test_timstamp}")
...
print(
    f"records in v1 but not in pre:                       {records_in_v1_not_in_pre.count()}"
)
print(
    f"records in v2 but not in pre:                       {records_in_v2_not_in_pre.count()}"
)
