...
for table in tables:
    ...
    df.write.mode("overwrite").format("delta").saveAsTable(
        f"{snapshot.target.snapshot_target_schema}.temp_{table}"
    )
...
for snapshot in config.snapshots.values():
    ...
    table_v2 = table + "_V2"
    spark.sql(
        f"""drop table if exists {snapshot.target.snapshot_target_schema}.{table_v1}"""
    )
    spark.sql(
        f"""drop table if exists {snapshot.target.snapshot_target_schema}.{table_v2}"""
    )
    spark.sql(
        f"""create table {snapshot.target.snapshot_target_schema}.{table_v1} 
              as select * from {snapshot.target.snapshot_target_schema}.temp_{table_v1}"""
    )
    spark.sql(
        f"""create table {snapshot.target.snapshot_target_schema}.{table_v2} 
...
for snapshot in config.snapshots.values():
    ...
    table_v2 = table + "_V2"
    spark.sql(
        f"""drop table if exists {snapshot.target.snapshot_target_schema}.temp_{table_v1}"""
    )
    spark.sql(
        f"""drop table if exists {snapshot.target.snapshot_target_schema}.temp_{table_v2}"""
    )
